...
public class NFShell {
	...
	public NFShell() {
		...
		System.out.println("NanoFiles terminal");
		System.out.println("Para ayuda, escribe 'help'");
	}
	...
	private String[] readGeneralCommandFromStdIn() {
		...
		while (true) {
			...
			default:
				...
				System.out.println("Ese comando solo es válido si estás dentro del browser");
				;
			}
			break;
		}
	...
	private String[] readBrowserCommandFromStdIn(NFConnector nfclient) {
		...
		while (true) {
			...
			default:
				skipValidateArgs = true;
				System.out.println("Ese comando solo es válido si estas fuera del buscador de ficheros");
			}
			break;
		}
		return vargs.toArray(args);
	...
	private boolean validateCommandArguments(String[] args) {
		...
		case NFCommands.COM_LOGIN:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Uso correcto: " + NFCommands.commandToString(command) + " <directory_server>");
				return false;
			}
			break;
		...
		case NFCommands.COM_BROWSE:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Uso correcto: " + NFCommands.commandToString(command) + " <remote_user>");
				return false;
			}
			break;
		...
		case NFCommands.COM_REGISTER:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Uso correcto:" + NFCommands.commandToString(command) + " <nickname>");
				return false;
			}
			break;
		...
		case NFCommands.COM_BGSERVE:
			if (args.length == 0 || args.length > 1) {
				System.out.println("Uso correcto:" + NFCommands.commandToString(command) + " <port>");
				return false;
			}
			break;
		// serve requiere el parámetro <port>
		case NFCommands.COM_DOWNLOAD:
			if (args.length != 2) {
				System.out.println(
						"Uso correcto:" + NFCommands.commandToString(command) + " <file_hash> <local_filename>");
				return false;
			}
			break;
		case NFCommands.COM_UPLOAD:
			if (args.length != 1) {
				System.out.println("Uso correcto:" + NFCommands.commandToString(command) + " <local_filename>");
				return false;
			}
			break;
